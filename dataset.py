...
class GlaS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_anno = torch.from_numpy(cv2.resize(cv2.imread('../GlaS/' + self.name + '_' + str(idx + 1) + '_anno.bmp'), (a, a), interpolation = cv2.INTER_NEAREST))
...
class Aug_GlaS(Dataset):
    ...
    def __len__(self):
        # q = 4
        list = glob.glob('../Aug_GlaS/' + self.name + '*anno*')
        return int(len(list))
